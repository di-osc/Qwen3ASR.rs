...
- Modify `vasr_models/Cargo.toml`: add optional `attention-rs` dependency and feature gates.
- Create `vasr_models/src/model/paged_kv_cache.rs`: preallocated page tensors, block table, slot mapping, and metadata helpers.
- Modify `vasr_models/src/model/mod.rs`: expose `paged_kv_cache` behind the feature.
- Modify `vasr_models/src/model/thinker_text.rs`: create paged attention objects per layer and add paged forward methods.
- Modify `vasr_models/src/model/thinker.rs`: expose a thinker-level paged forward method.
- Modify `vasr_models/src/model/generation.rs`: add an opt-in paged greedy path for batch size 1 dense prompts.
- Modify `vasr_models/src/inference/transcribe.rs`: choose paged generation only when feature/device/input constraints match.
...
```bash
git add vasr_models/Cargo.toml Cargo.lock
...
**Files:**
- Create: `vasr_models/src/model/paged_kv_cache.rs`
- Modify: `vasr_models/src/model/mod.rs`
...
```bash
cargo test -p vasr-models model::paged_kv_cache::tests::test_paged_cache_single_sequence_layout
```
...
```bash
cargo test -p vasr-models model::paged_kv_cache::tests
```
...
```bash
git add vasr_models/src/model/paged_kv_cache.rs vasr_models/src/model/mod.rs
...
**Files:**
- Modify: `vasr_models/src/model/thinker_text.rs`
...
```bash
git add vasr_models/src/model/thinker_text.rs
...
**Files:**
- Modify: `vasr_models/src/model/thinker.rs`
- Modify: `vasr_models/src/model/generation.rs`
...
```bash
cargo test -p vasr-models model::generation::tests
cargo check -p vasr-models --features metal-paged-attn
```
...
```bash
git add vasr_models/src/model/thinker.rs vasr_models/src/model/generation.rs
...
**Files:**
- Modify: `vasr_models/src/inference/transcribe.rs`
...
cargo test -p vasr-models --features metal-paged-attn
```
...
```bash
git add vasr_models/src/inference/transcribe.rs
