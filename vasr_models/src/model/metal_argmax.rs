...
pub fn argmax_token_id(logits: &Tensor) -> Result<u32> {
    ...
    let block_values =
        device.new_buffer(nblocks * MAX_K, DType::F32, "argmax-topk-block-values")?;
    let block_indices =
        device.new_buffer(nblocks * MAX_K, DType::U32, "argmax-topk-block-indices")?;
    let block_maxes = device.new_buffer(nblocks, DType::F32, "argmax-topk-block-maxes")?;
    let block_sums = device.new_buffer(nblocks, DType::F32, "argmax-topk-block-sums")?;
    let packed = device.new_buffer(2 * MAX_K + 2, DType::F32, "argmax-topk-packed")?;
    ...
    let packed = Tensor::from((
        Storage::Metal(MetalStorage::new(packed, device, 2 * MAX_K + 2, DType::F32)),
