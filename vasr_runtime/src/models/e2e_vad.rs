...
impl E2EVadModel {
    ...
    pub fn set_speech_noise_thres(&mut self, threshold: f32) {
        self.speech_noise_thres = threshold;
        self.opts.speech_noise_thres = threshold;
    }
    ...
    /// Offline segmentation matching `funasr_onnx.Fsmn_vad` + `E2EVadModel(online=false)`.
    ...
    /// Close any open streaming segment when no more audio is arriving.
    pub fn finalize_streaming(&mut self, max_end_silence_ms: i32) -> Vec<(u64, u64)> {
        self.max_end_sil_frame_cnt_thresh = max_end_silence_ms - self.opts.speech_to_sil_time_thres;
        if self.vad_state_machine == VadStateMachine::InSpeechSegment && self.frm_cnt > 0 {
            self.maybe_on_voice_end_if_last_frame(true, self.frm_cnt - 1);
        }
